...
Node.prototype.getHandler = function(method) {
    return this.handlers[method] || this.handlers['*'];
};
...
Node.prototype.override = function(node) {
    for (let key in node) {
        if (node.hasOwnProperty(key) && key !== 'path' && key !== 'pathLength' && key !== 'letterTable') {
            this[key] = node[key];
        }
    }
    for (let i in node.letterTable) {
        if (this.letterTable[i]) {
            // TODO: handle conflict when merging two routers.
            throw Error('Conflict occured when merging two routers.');
        }
        else this.letterTable[i] = node.letterTable[i];
    }
};
