...
class Router {
    ...
    callback() {
        ...
        return async function handler(ctx, next) {
            ...
            let handler = router.tree.locate(ctx.method, ctx.path, ctx.request.params);
            return yield* handler.call(this, ctx, next);
        }
    }
    ...
    register(method, path, handler) {
        ...
        if (method === '*' || path === '*') {
            ...
            this.tree.defaultHandler = composer.middleware('*', '/', router.layers);
            return;
        }

        // compose all middlewares
        const wrappedHandler = composer(method, path, handler, router.layers);
        router.tree.addHandler(method, path, wrappedHandler);
    }
    ...
    mergeWithChildRouter(mountPath, childRouter) {
        ...
        childTree.defaultHandler = composer.middleware('*', mountPath, router.layers, [childTree.defaultHandler]);
