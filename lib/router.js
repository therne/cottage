...
function RouterFactory(options) {
    ...
    var Router = function* Router(next) {
        ...
        try {
            ...
            var node = tree.locate(this.request.path, this.request.params);
            var handler = node ? node.handler : router.notFoundHandler;
    ...
    function register(method, path, handler) {
        ...
        if (method === '*' || path === '*') {
            ...
            if (path !== '*') handler = mount(path, handler);
    ...
    function registerRouter(path, handler) {
        ...
        // FIXME: child notFound handler is omitted.
        subrouter.trees.forEach(function(tree, method) {
            var ourTree = router.trees.get(method);
            if (!ourTree) {
                ourTree = new Tree(options);
                router.trees.set(method, ourTree);
            }
            let subTree = tree.clone();
            subTree.setRootPath(path);
            subTree.traverse(function(nodePath, node) {
                // compose parent middleware into child node
                node.handler = composer(method, nodePath, node.handler, router.layers, true);
            });
            ourTree.merge(path, subTree);
        });
    ...
    function registerMultiple(method, path, args, middlewareOnly) {
        var handlers = [];
        for (let i in args) {
            if (args[i] instanceof Array) {
                handlers = handlers.concat(
                    args[i].filter(function(f) { return typeof f !== 'function' }));
            }
    ...
    Router.use = function(path, middlewares) {
        ...
        registerMultiple('*', path, arguments, true);
    }
    ...
    http.METHODS.forEach(function(method) {
        ...
        Router[method.toLowerCase()] = function(path, callback) {
            registerMultiple(method, path, arguments, false);
        };
    });
