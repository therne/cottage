...
function RouterFactory(options) {
    ...
    router.tree.defaultHandler = router.notFoundHandler;
    router.layers.push(new Layer('*', '/', router.notFoundHandler));
    ...
    var Router = function* Router(next) {
        ...
        try {
            var handler = router.tree.locate(this.method, this.path, this.request.params);
    ...
    function register(method, path, handler) {
        ...
        if (method === '*' || path === '*') {
            ...
            if (path !== '*') {
                ...
                router.tree.addMiddleware(method, path, composer.middleware('*', path, router.layers, [handler]));
            }
    ...
    Router.setNotFoundHandler = function(handler) {
        ...
        router.layers[0].handler = handler;
        router.tree.defaultHandler = composer.middleware('*', '/', router.layers);
    }
